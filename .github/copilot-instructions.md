...
### 不具合修正後の self-inspect.ps1 更新ルール

**重要**: バグ修正・デグレ対応を行った場合、同じ不具合が再発しないよう `.github/hooks/self-inspect.ps1` に検出ルールを追加すること。

#### いつ追加するか

| 修正内容 | self-inspect への追加 |
|----------|----------------------|
| CSS レイアウトのデグレ修正 | 崩れパターンを検出する CSS ルール (R3〜R5 系) |
| API エラーハンドリング漏れ | catch 句の不備を検出するルール (R2 系) |
| Cosmos DB 接続パターンの誤用 | 危険な呼び出しパターンを検出するルール (R1 系) |
| その他の再発しやすいパターン | 新たな Rn ルールとして追加 |

#### 追加フロー

1. バグを修正する
2. 同じパターンが再発しないよう `.github/hooks/self-inspect.ps1` に検出ルールを追加
3. `pwsh .github/hooks/self-inspect.ps1 -Mode start` でルールが正しく動作することを確認
4. バグ修正と self-inspect 更新を**同一コミット・同一PR**に含める
