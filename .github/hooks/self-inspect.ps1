...
#   R43. Copilot OTel の captureContent が tracked 設定で既定有効になるパターン
#   R44. devcontainer 起動が生成物 Langfuse compose やホスト側 Node セットアップに依存するパターン
#
# 引数:
...
# ---------------------------------------------------------------------------
# R43: Copilot OTel は安全な既定値と再現性を維持すること
#      (tracked 設定で会話本文 capture を有効化しない / 外部 compose はタグ固定 / self-inspect を必須実行)
# ---------------------------------------------------------------------------
$otelTrackedConfigFiles = @(
    (Join-Path $RepoRoot '.vscode\settings.json')
    (Join-Path $RepoRoot '.devcontainer\devcontainer.json')
    (Join-Path $RepoRoot '.devcontainer\docker-compose.yml')
)

foreach ($path in $otelTrackedConfigFiles) {
    if (-not (Test-Path $path)) { continue }
    $raw = Get-Content -LiteralPath $path -Raw
    if ($raw -match '"github\.copilot\.chat\.otel\.captureContent"\s*:\s*true' -or
        $raw -match 'COPILOT_OTEL_CAPTURE_CONTENT\s*:\s*["'']true["'']') {
        Add-Finding -Rule 'R43-copilot-otel-safe-defaults' -Severity 'High' `
            -File $path `
            -Detail 'tracked 設定では Copilot Chat の captureContent を既定 false にし、会話本文・ツール引数の記録は必要時だけ opt-in にしてください'
    }
}
...
if (Test-Path $readmePath) {
    ...
    if ($raw -match 'export\s+\$\(grep\s+OTEL_EXPORTER_OTLP_HEADERS\s+\.env\)') {
        Add-Finding -Rule 'R43-copilot-otel-safe-defaults' -Severity 'Medium' `
            -File $readmePath `
            -Detail 'OTEL_EXPORTER_OTLP_HEADERS は空白を含むため export $(grep ...) ではなく、sed 等で値部分をクォート付きで代入してください'
    }
}

$otelSetupScript = Join-Path $RepoRoot 'scripts\setup-copilot-otel.mjs'
if (Test-Path $otelSetupScript) {
    $raw = Get-Content -LiteralPath $otelSetupScript -Raw
    if ($raw -match 'raw\.githubusercontent\.com/langfuse/langfuse/main/docker-compose\.yml') {
        Add-Finding -Rule 'R43-copilot-otel-safe-defaults' -Severity 'Medium' `
            -File $otelSetupScript `
            -Detail 'Langfuse compose の取得元は main ではなく既知のタグまたはコミット SHA に固定してください'
    }
}

$preCommitHook = Join-Path $RepoRoot '.husky\pre-commit'
if (Test-Path $preCommitHook) {
    $raw = Get-Content -LiteralPath $preCommitHook -Raw
    if ($raw -match 'pwsh が見つからないため.*self-inspect をスキップ' -or
        $raw -match 'self-inspect は PowerShell 利用可能な環境で実行してください') {
        Add-Finding -Rule 'R43-copilot-otel-safe-defaults' -Severity 'High' `
            -File $preCommitHook `
            -Detail 'pwsh 不在時に self-inspect をスキップしてコミットを許可しないでください。PowerShell を導入するかコミットを失敗させてください'
    }
}

$devcontainerDockerfile = Join-Path $RepoRoot '.devcontainer\Dockerfile'
if (Test-Path $devcontainerDockerfile) {
    $raw = Get-Content -LiteralPath $devcontainerDockerfile -Raw
    if ($raw -notmatch 'packages\.microsoft\.com/config/debian/12/packages-microsoft-prod\.deb' -or
        $raw -notmatch 'apt-get install[^\r\n]*powershell') {
        Add-Finding -Rule 'R43-copilot-otel-safe-defaults' -Severity 'Medium' `
            -File $devcontainerDockerfile `
            -Detail 'devcontainer では pre-commit の self-inspect を実行できるよう PowerShell (pwsh) を導入してください'
    }
}

# ---------------------------------------------------------------------------
# R44: devcontainer の既定起動は未生成 OTel / Langfuse 資材に依存しないこと
#      (初回起動で langfuse/docker-compose.yml や .env が無くても workspace を起動できる)
# ---------------------------------------------------------------------------
$devcontainerJson = Join-Path $RepoRoot '.devcontainer\devcontainer.json'
if (Test-Path $devcontainerJson) {
    $raw = Get-Content -LiteralPath $devcontainerJson -Raw
    if ($raw -match '\.\./langfuse/docker-compose\.yml' -or
        $raw -match '"initializeCommand"\s*:\s*"[^"]*setup-copilot-otel' -or
        $raw -match '"postStartCommand"\s*:\s*"[^"]*start-copilot-otel-session') {
        Add-Finding -Rule 'R44-devcontainer-otel-optional' -Severity 'High' `
            -File $devcontainerJson `
            -Detail 'devcontainer の既定起動は生成物の Langfuse compose、.env、ホスト側 Node セットアップに依存させず、OTel は npm run otel:compose で opt-in 起動してください'
    }
}

$devcontainerCompose = Join-Path $RepoRoot '.devcontainer\docker-compose.yml'
if (Test-Path $devcontainerCompose) {
    $raw = Get-Content -LiteralPath $devcontainerCompose -Raw
    if ($raw -match '(^|\r?\n)\s*otel-collector\s*:' -or
        $raw -match 'langfuse-web' -or
        $raw -match 'env_file\s*:\s*\r?\n\s*-\s*\.\./\.env') {
        Add-Finding -Rule 'R44-devcontainer-otel-optional' -Severity 'High' `
            -File $devcontainerCompose `
            -Detail 'devcontainer の workspace compose は未追跡 .env や Langfuse/OTel サービスを必須依存にせず、初回起動をブロックしない構成にしてください'
    }
}
...
if ($findings.Count -eq 0) {
    Write-Host "✅ 検出された不整合はありません (R1 / R2 / R3 / R4 / R5 / R6 / R7 / R8 / R9 / R10 / R11 / R12 / R13 / R14 / R15 / R16 / R17 / R18 / R19 / R20 / R21 / R22 / R23 / R24 / R24b / R25 / R26 / R27 / R28 / R29 / R30 / R31 / R32 / R33 / R34 / R35 / R36 / R37 / R38 / R39 / R40 / R41 / R42 / R43 / R44)"
    exit 0
}
