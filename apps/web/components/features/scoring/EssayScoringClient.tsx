...
export function EssayScoringClient({ samples }: EssayScoringClientProps): JSX.Element {
  const [questionId, setQuestionId] = useState(samples[0]?.questionId ?? '');
  const [answer, setAnswer] = useState({ setsumonA: '', setsumonI: '', setsumonU: '' });
  const stream = useScoringStream();

  const current = samples.find((s) => s.questionId === questionId) ?? samples[0];

  const perspectives: (PerspectiveCardData & { subQuestion?: string })[] = stream.events
  ...
  const errors = stream.events
    ...
    .map((e) => e.data as { id: string; message: string; subQuestion?: string });
  ...
  const subSections: { key: 'A' | 'I' | 'U'; label: string; field: 'setsumonA' | 'setsumonI' | 'setsumonU' }[] = [
    ...
    <main className={styles.page}>
      <div className={styles.wrap}>
        <header className={styles.header}>
          ...
          <p className={styles.sub}>設問ア・イ・ウを個別に入力し、AI採点を受けられます。</p>
        </header>

        <section className={styles.card}>
          <div className={styles.cardHead}>
            <span className={styles.cardLabel}>テーマ</span>
            {current && <span className={styles.limit}>{current.examType}午後II</span>}
          </div>
          ...
          {current && <p style={{ marginBottom: 8, lineHeight: 1.8 }}>{current.theme}</p>}
        </section>
          ...
          subSections.map((s) => {
            ...
            return (
              <section key={s.key} className={styles.card}>
                ...
                <p style={{ marginBottom: 8, fontSize: 13, color: '#4a4238' }}>{meta.requirements}</p>
                <FreeformInput
                  value={answer[s.field]}
                  onChange={(v) => setAnswer((prev) => ({ ...prev, [s.field]: v }))}
              ...
              return (
                <div key={sub} className={styles.subSection}>
                  ...
                  {subErrs.map((e) => (
                    <PerspectiveCard
                      key={`e-${sub}-${e.id}`}
                      data={{ id: e.id, name: e.id, score: 0, subQuestion: sub }}
                      variant="essay"
                      error={e.message}
                    />
                  ))}
            ...
            {complete && complete.overallRank && (
              <div className={styles.summary}>
                <div>
                  ...
                  <div className={styles.totalScore}>{complete.overallScore}</div>
                </div>
                <div className={`${styles.rank} ${styles[complete.overallRank]}`} aria-label="総合ランク">
