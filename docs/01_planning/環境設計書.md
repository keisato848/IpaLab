...
| :--- | :--- | :--- | :--- |
| **Static Web Apps** | `swa-pm-exam-dx-prod` | Next.jsフロントエンドのホスティング、グローバルCDN | Standard (商用) / Free (開発) |
| **Functions** | `func-pm-exam-dx-api` | バックエンドAPIロジック (Node.js) | Consumption (従量課金) |
| **Cosmos DB** | `cosmos-pm-exam-dx-db` | NoSQLデータベース (JSONドキュメント) | Serverless (Request Units) |
| **Entra ID B2C** | `pmexamdx.onmicrosoft.com` | ユーザーID管理、認証・認可 (OIDC/OAuth2) | External Identities Monthly Active Users (MAU) |
...
| **Blob Storage** | `stpmexamdxdata` | 論文データ（暗号化済）、静的アセット | Standard LRS / Hot |
...
graph TB
    ...
    Browser -->|"OIDC / Auth Code Flow"| Auth
    Mobile -->|"OIDC / Auth Code Flow"| Auth
...
1. **Build:** Next.js (Web) / Expo (Prebuild) / Functions (Build)
2. **Test:** Unit Tests (Jest), Lint (ESLint)
3. **Deploy:**
   - Web -> Azure Static Web Apps (Oryx build or Pre-built artifacts)
   - API -> Azure Functions Deployment
...
### 4.1 認証・認可
- **認証:** Entra ID B2C を利用したソーシャルログイン (GitHub, Google) およびメール認証。
- **認可:** JWT (JSON Web Token) の検証を Azure Functions 側で実施（またはSWAの認証機能を利用）。
