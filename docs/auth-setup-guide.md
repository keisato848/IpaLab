...
### Q. "Configuration" エラーが表示される (`/api/auth/error?error=Configuration`)
**原因**: 環境変数不足、またはデータベース初期化未完了の可能性があります。

**対策1: 環境変数の追加 (`AUTH_TRUST_HOST`, `AUTH_SECRET`)**
Azure Static Web Apps では、以下の変数が必須になる場合があります。
- `AUTH_TRUST_HOST`: `true`
- `AUTH_SECRET`: (設定済みか再確認してください)

**対策2: データベースの初期化 (コンテナ作成)**
機能追加ブランチ (`feature/pm-exam-support`) がまだマージ・デプロイされていない場合、データベースに必要なコンテナ (`Users`, `Accounts` 等) が存在せず、ログイン時にエラーになります。
以下のコマンドを手元のターミナルで実行し、Azure上のDBを初期化してください。

```powershell
# Azureの接続文字列をセットして初期化関数を実行
$env:COSMOS_DB_CONNECTION='(Azureの接続文字列)'; cd apps/web; npx ts-node -e "require('./lib/services/cosmos').initDatabase().then(() => console.log('Done')).catch(e => console.error(e))"
```
